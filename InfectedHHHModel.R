...
# create a 42*21 matrix for all the Infected cases in the counties
matrix1 <- data.matrix(covid.df[covid.df$Region == "Blekinge",]$No.of.cases, rownames.force = FALSE)
matrix2 <- data.matrix(covid.df[covid.df$Region == "Dalarna",]$No.of.cases, rownames.force = FALSE)
matrix3 <- data.matrix(covid.df[covid.df$Region == "Gavleborg",]$No.of.cases, rownames.force = FALSE)
matrix4 <- data.matrix(covid.df[covid.df$Region == "Gotland",]$No.of.cases, rownames.force = FALSE)
matrix5 <- data.matrix(covid.df[covid.df$Region == "Halland",]$No.of.cases, rownames.force = FALSE)
matrix6 <- data.matrix(covid.df[covid.df$Region == "Jamtland",]$No.of.cases, rownames.force = FALSE)
matrix7 <- data.matrix(covid.df[covid.df$Region == "Jonkoping",]$No.of.cases, rownames.force = FALSE)
matrix8 <- data.matrix(covid.df[covid.df$Region == "Kalmar",]$No.of.cases, rownames.force = FALSE)
matrix9 <- data.matrix(covid.df[covid.df$Region == "Kronoberg",]$No.of.cases, rownames.force = FALSE)
matrix10 <- data.matrix(covid.df[covid.df$Region == "Norrbotten",]$No.of.cases, rownames.force = FALSE)
matrix11 <- data.matrix(covid.df[covid.df$Region == "Orebro",]$No.of.cases, rownames.force = FALSE)
matrix12 <- data.matrix(covid.df[covid.df$Region == "Ostergotland",]$No.of.cases, rownames.force = FALSE)
matrix13 <- data.matrix(covid.df[covid.df$Region == "Skane",]$No.of.cases, rownames.force = FALSE)
matrix14 <- data.matrix(covid.df[covid.df$Region == "Sodermanland",]$No.of.cases, rownames.force = FALSE)
matrix15 <- data.matrix(covid.df[covid.df$Region == "Stockholm",]$No.of.cases, rownames.force = FALSE)
matrix16 <- data.matrix(covid.df[covid.df$Region == "Uppsala",]$No.of.cases, rownames.force = FALSE)
matrix17 <- data.matrix(covid.df[covid.df$Region == "Varmland",]$No.of.cases, rownames.force = FALSE)
matrix18 <- data.matrix(covid.df[covid.df$Region == "Vasterbotten",]$No.of.cases, rownames.force = FALSE)
matrix19 <- data.matrix(covid.df[covid.df$Region == "Vasternorrland",]$No.of.cases, rownames.force = FALSE)
matrix20 <- data.matrix(covid.df[covid.df$Region == "Vastmanland",]$No.of.cases, rownames.force = FALSE)
matrix21 <- data.matrix(covid.df[covid.df$Region == "Vastra Gotaland",]$No.of.cases, rownames.force = FALSE)


Infected.matrix = cbind(matrix1, matrix2, matrix3, matrix4, matrix5, matrix6, matrix7, matrix8, matrix9, matrix10, matrix11, matrix12, matrix13, matrix14, matrix15, matrix16, matrix17, matrix18, matrix19, matrix20, matrix21) 
colnames(Infected.matrix) = c('Blekinge','Dalarna','Gavleborg','Gotland','Halland','Jamtland','Jonkoping','Kalmar','Kronoberg','Norrbotten','Orebro','Ostergotland','Skane','Sodermanland','Stockholm','Uppsala','Varmland','Vasterbotten','Vasternorrland','Vastmanland','Vastra Gotaland')
...
colnames(Infected.matrix) = gsub(" ", ".",colnames(Infected.matrix))
...
# fit a HHH model
covid.fit.Infected <-  hhh4(
  stsObj = Infecteds.sts, 
  control = list(
    ar = list(f = ~ 1),
    ne =  list(f = ~ 1, weights = neighbourhood(Infecteds.sts) == 1),
    end = list(f = ~ t+t2+P1+P2+P3+P4+P5+P6+P7+P8+P9+temp1+temp2, offset = population(Infecteds.sts)),
    family = "NegBin1"
  )
)

summary(covid.fit.Infected)


fitted <- plot(covid.fit.Infected, type = "fitted", total = TRUE, hide0s = TRUE, par.settings = NULL, legend = TRUE, ylab = 'Infected Case' ) 
fitted$Overall[12:20,]
colSums(fitted$Overall)[3:5] / sum(fitted$Overall[,1])

#subplot <- which(colSums(observed(Infecteds.sts)) > 200)
subplot <- which(colnames(observed(Infecteds.sts)) == 'Stockholm')
Stockholm <- plot(covid.fit.Infected, ylab = 'Infected Case', type = "fitted", units = subplot, hide0s = TRUE, legend = TRUE, par.settings = NULL)
subplot <- which(colnames(observed(Infecteds.sts)) == 'Vastra.Gotaland')
Vastra.Gotaland <- plot(covid.fit.Infected, ylab = 'Infected Case', type = "fitted", units = subplot, hide0s = TRUE, legend = TRUE, par.settings = NULL)
subplot <- which(colnames(observed(Infecteds.sts)) == 'Skane')
Skane <- plot(covid.fit.Infected, ylab = 'Infected Case', type = "fitted", units = subplot, hide0s = TRUE, legend = TRUE, par.settings = NULL)
subplot <- which(colnames(observed(Infecteds.sts)) == 'Dalarna')
Dalarna <- plot(covid.fit.Infected, ylab = 'Infected Case', type = "fitted", units = subplot, hide0s = TRUE, legend = TRUE, par.settings = NULL)
...
prediction <- predict(covid.fit.Infected) 
